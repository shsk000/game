...
> ⚠️ T-27〜T-31 は実機で 30 分プレイが必要なため、ユーザー側で実施。
> 自動化できる範囲（tsc / build / smoke / 画面スナップショット）は Phase 5 で実行済み。

- [ ] **T-27** 通しプレイ 30 分（初回、リセット状態から） ※ユーザー実機検証
- [ ] **T-28** 数値計測：リリース本数 / スコア帯分布 / 神ゲー出現 / 収支推移 / 規模解放まで何分
- [ ] **T-29** balance-design §8 の合格基準を満たすか判定
  - リリース 8〜12 本
  - 致命的失敗 ≒ 20%、失敗 ≒ 35%（±5%）
  - 神ゲー 0〜1 回
  - 30 分以内に mobile 解放可能
- [ ] **T-30** 不合格項目の調整（`balance.ts` を再校正） ※balance.ts の早見表に従って `BALANCE_README.md` を参照
- [ ] **T-31** 再度通しプレイ → 合格まで反復
...
| Phase 3 UI | 6 | ✅ 完了 |
| Phase 4 検証 | 5 | ⏸ 実機プレイ待ち |
| Phase 5 仕上げ | 5 | 3/5（commit/PR は承認後） |
| **合計** | **36** | **29/36** |
