...
- [ ] **A-10** タイピング非開発中もアイドル進行（オフィス画面で任意で1週送りボタン）※ 後段検討、まずは開発中のみ進行で OK 判定
- [ ] **A-11** ユニットテスト：`addWeeks`, `formatDate`, `monthChanged` の境界条件（12月→1月、第4週→第1週）
...
- [ ] **B-10** ヒット帯の閾値（70=ヒット ×4 / 85=大ヒット ×10 / 95=神ゲー ×30）
- [~] **B-11** 神ゲーガチャ 5% — D-9 と統合（演出のみ確認、実プロブ未検証）
- [ ] **B-12** 資金 0 でゲームオーバー画面
- [ ] **B-13** 利益計算ユーティリティ `utils/profit.ts`（売上 − 開発費 − 期間月数 × 月固定費）を新設＋テスト ※ 実装は ReleaseScreen 直書きで動作確認済、ファイル分離は未

### C. 品質計算式リテイク（4要素ウェイト） 【P0】

> spec.md §2 / diagnosis §1-3, §2-2「採用が効かない」を解消する最重要タスク

- [ ] **C-1** `utils/metascore.ts` の `computeQuality` を 4要素ウェイト方式に書き換える
  ```
  quality_base = キャラ能力 × 0.50
               ...
               + 0.10
  ```
- [ ] **C-2** `utils/character.ts`（新設）：`computeCharacterScore({ assignedEmployees, scale, project })` を実装
  - 基礎 power 合計 → 0〜100 正規化
  - 役割マッチ +30%（programmer×gameplay/innovation 等）
  - specialty 一致 +20% / 一致
  - 複数人相乗：2人 +10% / 3人 +25%
  - 規模適合：人数 OK → +5% / 不足 → -20%
- [ ] **C-3** `utils/character.ts` にタイピングスコア計算 `computePerformanceScore` を強化
  - WPM 100 を基準に ±20%
  - コンボ 30+ +5% / 100+ +10% / 200+ +15%
  - 精度 95% 未満で -5〜-20%
  - バグなし完走 +5%
- [ ] **C-4** `utils/compatibility.ts` のジャンル相性スコアを 0〜100 正規化に対応するヘルパー追加
- [ ] **C-5** トレンド補正 ×1.3 / ×1.7、新規開拓 ×1.3 を `final_quality` 段階で乗算
- [ ] **C-6** 運乱数 0.9〜1.1（±10%）と神ゲーガチャ ×1.5 を `final_quality` に統合
- [ ] **C-7** `Work.breakdown` を新型 `{ character, affinity, performance, luck, trendMul, pioneer, gachaHit }` に拡張＋セーブマイグレーション
- [ ] **C-8** ユニットテスト：4要素ウェイトの組合せで「キャラのみ強い時」「演技のみ強い時」のスコア順位を assert
...
- [ ] **E-4** 「予想売上 − 開発費 − 固定費」で **「予想利益」** を出して赤字なら警告色
...
- [ ] **F-5** 時短発生（タイピング WPM 100+ 等）時に **「-X 週」** テロップ
- [ ] **F-6** 完成時に **「開発期間 6 ヶ月 2 週（= 26 週）」** のサマリ表示
...
- [ ] **J-1** `utils/storage.ts` の `KEY` を `typing-factory:v5` にアップグレード（v4 を LEGACY_KEY_V4 として保持）
- [ ] **J-2** `migrateFromV4` を実装：
  - `funds` を **×100** スケール（簡略：v0.9 の ¥1,500 → v0.10 の ¥150,000）※ 仕様 §6 では「売上×10000」だが実機桁の妥当性をテストしてから採用
  - `library[].totalRevenue` / `initialRevenue` / `salesPool` を **×10,000** にスケール
  - `lifetimeRevenue` も同様にスケール
  - `currentDate = { year: 2026, month: 1, week: 1 }` を初期値で追加
  - `ownedEquipment = []` を追加
  - `employees[].monthlyWage` を `power` に応じてマップ
- [ ] **J-3** マイグレーション後の `Persisted` を `version: 5` で再保存
- [ ] **J-4** マイグレーション失敗時のフォールバック（`reset()` → 新規開始）
...
- [ ] **O-4** OfficeView のスプライト 404 修正：`public/sprites/office/` 配下に家具・キャラ画像が無く黒くなる。fallback CSS（emoji 等）を実装するか、本素材生成
- [ ] **O-5** `npx playwright test` 全件 GREEN 確認（v10-screens.spec.ts + 既存 smoke + office-screenshot）
- [ ] **O-6** C-1 の判定見直し（実装は `computeQualityV10` 新規 export として完了、既存 `computeQuality` 温存）→ tasks.md の C-1 を [x] に
- [ ] **O-7** B-10（ヒット帯倍率 70/85/95 × 4/10/30）を `sales.ts` に反映
- [ ] **O-8** B-12（資金 0 → ゲームオーバー）を実装。借金システム（spec §9-2）とセットで
- [ ] **O-9** D-2 / D-5 / D-8（StatusBar に formatYen、Library/Release のレーダー）を完成
- [ ] **O-10** F-3/F-4/F-5/F-6（完成時サマリテロップ、月初テロップ、神ゲーカットイン、バグ累計）
