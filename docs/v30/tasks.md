...
## 3.6 打鍵文の日本語校閲（敵対レビュー反映）

- [x] 3.6-1 全480件をカテゴリ別に敵対レビュー（並列4レビュアー＋自己検証）
- [x] 3.6-2 Tier A 明確な誤り・非語 7件を修正
- [x] 3.6-3 Tier B 非標準語・造語・不自然 8件を修正
- [x] 3.6-4 Tier D 専門ジャーゴン 30件を平易語へ置換
- [x] 3.6-5 置換後もひらがな限定・各120件ユニーク（`phrasePools.test.ts` 緑）
- [x] 3.6-6 Tier C（同綴誤読 めいど=明度/さいど=彩度/ずおーだー/しんせ/ちょう=調/きゅうふ=休符/おかず=フィル）7件を明色語へ置換
