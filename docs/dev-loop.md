...
|---|---|---|---|---|
| **責任者 / Producer** | `producer.md` | ループ頂点（何を作るか） | game-design（北極星）+ roadmap/gaps | 読み取り専用（コード変更しない） |
...
- **並列タスク処理（worktree 分離）は既定オフ**。ブランチ上で逐次処理し、統合とコミットを単純に保つ。
  独立タスクを並列化したい場合は `parallel=true`（実験的：各実装を worktree 隔離、統合が複雑化する）。
- 「複数エージェント」は主に**1タスクを5ロール＋審査3体で回す縦の多エージェント**で満たす。
  横の並列は副次オプション。
